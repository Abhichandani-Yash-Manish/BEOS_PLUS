...
const DonorDashboard: React.FC = () => {
    ...
    useEffect(() => {
        ...
        socket.on('new-request', (newRequest: BloodRequest) => {
        ...
        socket.on('crisis-mode', (data: any) => {
            if (data.active) {
    ...
    const fetchDashboardData = async () => {
        try {
            ...
            setProfile(profileRes as unknown as Donor); // api returns wrapped data but getMyDonorProfile type says Donor? checked api.ts, it returns Promise<Donor> directly if using getMyDonorProfile = ... fetchAPI<Donor> but fetchAPI usually returns data object 
            // Wait, api.ts definition: export const getMyDonorProfile = () => fetchAPI<Donor>('/api/donors/me');
            // But server returns { success: true, data: donor }. 
            // My api.ts fetchAPI return type is T. 
            // If the server wraps it, T should be { data: Donor }.
            // The previous code used profileRes.data.
            // Let's assume for now api.ts needs adjustment or use 'any' to bridge until verified.
            // Actually, looking at Login, response.user works. 
            // Let's cast to any here to be safe and match the existing logic `setProfile(profileRes.data)`
            ...
            const requestsData = requestsRes.data || requestsRes;
             ...
             // @ts-ignore
            const nearby = requestsData.filter((r: any) =>
                 // @ts-ignore
                r.hospital_city?.toLowerCase() === (profileRes.data || profileRes).city?.toLowerCase()
            );
            setRequests(nearby);
    ...
    const handleUpdateProfile = async (data: Partial<Donor>) => {
        if (!profile) return;
        try {
    ...
    const handleRespondToEmergency = (request: BloodRequest) => {
                                            ...
                                            <div>
                                                <div className="flex items-center gap-2 mb-2">
                                                    ...
                                                    <span className="text-sm text-gray-400">
                                                        {req.fulfilled_at ? new Date(req.fulfilled_at).toLocaleDateString() : 'Unknown'}
                                                    </span>
                                                </div>
