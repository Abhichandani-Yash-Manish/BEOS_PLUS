...
import { BloodBatch, BloodType } from '../types/models';
...
export const AddBatchForm: React.FC<AddBatchFormProps> = ({ onSubmit, loading }) => {
...
export const InventoryTable: React.FC<InventoryTableProps> = ({ batches, onDelete }) => {
    if (!batches.length) {
                    ...
                    {batches.map(batch => {
                        ...
                        return (
                            <tr key={batch.id} className={`border-b border-gray-800 ${status.class} hover:bg-white/5`}>
                                ...
                                <td className="p-3 text-sm text-gray-400">{batch.created_at ? new Date(batch.created_at).toLocaleDateString() : 'Unknown'}</td>
...
export const LowStockAlerts: React.FC<LowStockAlertsProps> = ({ batches }) => {
