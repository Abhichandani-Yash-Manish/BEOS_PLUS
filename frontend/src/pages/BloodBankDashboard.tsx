...
import { BloodBank, BloodBatch, Donor, BloodType } from '../types/models';
import './BloodBankDashboard.css';

const BloodBankDashboard: React.FC = () => {
    const { showToast } = useToast();
    const socket = useSocket();
    const [profile, setProfile] = useState<BloodBank | null>(null);
    const [batches, setBatches] = useState<BloodBatch[]>([]);
    ...
    const fetchDashboardData = async () => {
        try {
            ...
            // @ts-ignore
            setBatches(batchesRes.data || batchesRes);

            // Fetch donors for the radar
            // @ts-ignore
            const donorsRes = await getDonors({ city: (profileRes.data || profileRes).city });
            // @ts-ignore
            setDonors(donorsRes.data || donorsRes);
    ...
    const getStats = () => {
        ...
        const expiringSoon = validBatches.filter(b => {
            const days = Math.ceil((new Date(b.expiry_date).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24));
            return days <= 7;
        }).length;
