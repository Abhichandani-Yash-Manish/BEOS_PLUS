...
// @ts-ignore - Assuming this JSON exists
...
const ProgressSteps: React.FC<{ currentStep: number; steps: typeof STEPS }> = ({ currentStep, steps }) => {
    return (
...
interface StepPatientProps extends StepProps {
    onDetectLocation: () => void;
    detectingLocation: boolean;
}

const StepPatient: React.FC<StepPatientProps> = ({ formData, setFormData, onNext, onDetectLocation, detectingLocation }) => {
...
const StepBlood: React.FC<StepProps> = ({ formData, setFormData, onNext, onBack }) => {
...
const StepClinical: React.FC<StepProps> = ({ formData, setFormData, onNext, onBack }) => {
    return (
        <motion.div
            className="wizard-step"
            initial={{ opacity: 0, x: 20 }}
        ...
        >
            ...
            <div className="form-grid">
                ...
                <div className="form-group">
                    <label className="critical-checkbox-wrapper">
                        <input
                            type="checkbox"
                        ...
                        <div className="checkbox-box">
                            {formData.is_critical && <Check size={14} strokeWidth={4} />}
                        </div>
                        <span className="checkbox-label">CRITICAL CONDITION?</span>
                    </label>
                    <p className="field-hint pl-8">Patient requires immediate attention (within 1 hour).</p>
                </div>
...
const StepSafety: React.FC<StepProps> = ({ formData, setFormData, onNext, onBack }) => {
    const handleReactionToggle = (type: string) => {
...
const StepSuccess: React.FC<StepSuccessProps> = ({ formData, onClose }) => {
    return (
...
interface EmergencyRequestWizardProps {
    onClose: () => void;
    onSubmit: (data: WizardFormData) => Promise<void>;
}

export default function EmergencyRequestWizard({ onClose, onSubmit }: EmergencyRequestWizardProps) {
