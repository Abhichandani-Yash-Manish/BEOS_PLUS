...
import { BloodBatch, Donor, BloodType } from '../types/models';
...
export const InventoryHeatmap: React.FC<InventoryHeatmapProps> = ({ batches }) => {
    const [selectedType, setSelectedType] = useState<BloodType | null>(null);
    const bloodTypes: BloodType[] = ['A+', 'A-', 'B+', 'B-', 'AB+', 'AB-', 'O+', 'O-'];

    // Calculate totals per blood type (only non-expired)
    const getStockData = () => {
        const data: Record<string, { total: number, expiringSoon: number, batches: number }> = {};
        bloodTypes.forEach(type => {
            ...
            const expiringSoon = validBatches.filter(b => {
                const daysToExpiry = Math.ceil((new Date(b.expiry_date).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24));
...
export const ExpiryAlertsList: React.FC<ExpiryAlertsListProps> = ({ batches }) => {
...
interface DonorRadarProps {
    donors: Donor[];
    selectedBloodType: BloodType | null;
    onSelectDonor?: (donor: Donor) => void;
}

export const DonorRadar: React.FC<DonorRadarProps> = ({ donors, selectedBloodType, onSelectDonor }) => {
